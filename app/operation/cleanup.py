...
class Cleaner(object):
    ...
    async def static_revert(self, obj, cmd: str) -> None:
        await directed_cleanup(self.op_log, obj.host, cmd, self.run_on_agent)

    async def revert_domain_pass(self, obj: Trashed) -> None:
        await fix_domain_pass(self.op_log, obj.host, obj.hit_user, obj.hit_password, obj.domain,
                              self._server_operation, self.delete)

    async def revert_local_pass(self, obj: Trashed) -> None:
        await fix_local_pass(self.op_log, obj.host, obj.hit_user, obj.hit_password, self.run_on_agent)
...
async def directed_cleanup(op: Operation, target_host: ObservedHost, CleanCmd: str, job_run_until_success):
    await job_run_until_success(target_host, *static.cleanupCMD(CleanCmd))


async def fix_domain_pass(op: Operation, target_host: ObservedHost, user: str, password: str, domain: str, srv_op, del_fun):
    core = await spawn_recovery_rat(target_host, user, "CalderaRulez%123", domain, srv_op)
    await rat_operation(srv_op, core, srv_op._interface.powershell_function, "footprint",
                        PSFunction("Password-Mayhem-Clean", PSArg("userID", user),
                                   PSArg('pass', password, escape=escape_string_literally)))
    loc = ObservedFile(**{'path': 'C:\cleanup.exe', 'host': target_host, 'use_case': 'dropped'}).save()
    await clean_recovery_rat(core, loc, del_fun)
...
async def repair_files(OP: Operation, host: ObservedHost, job_run_until_success) -> bool:
    await job_run_until_success(host, *footprint.files())
    return True


async def fix_local_pass(OP: Operation, host: ObservedHost, user: str, password: str, job_run_until_success) -> bool:
    await job_run_until_success(host, *footprint.password(user, password))
    return True
