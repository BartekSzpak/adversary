...
class PlannerContext(object):
    ...
    def __init__(self, logic: LogicContext, rec_limit, performed_actions=None, logger=None):
        self.logic = logic
    ...
    def plan(self, time_limit: Union[int, None], max_depth: int, max_returns: int = 500000) -> List:
        ...
        self.logger.info("Processed " + str(num_plans) + " plans")
    ...
    def _recursive_planner(self, time_limit: Union[int, None], total_depth: int, current_depth: int,
        ...
        Args:
            ...
            ignore_actions: the set of action_args which should not be followed
        ...
        if len(optimal_action_args_by_action):
            self.logger.info("found {} mathematically optimal actions".format(len(optimal_action_args_by_action)))
        ...
        for action, t in roundrobin(*action_args_by_action.values()):
            ...
            if identified_plans and time_limit and time.process_time() - start_time >= time_limit:
                self.logger.info("Planning time limit reached, returning {} plan(s)".format(len(identified_plans)))
                break
        ...
        if total_depth == current_depth:
            self.logger.info("Planning run took {} seconds and returned {} actions".format(t1 - start_time, sum(
            ...
            self.logger.info("Optimization pruning took {} seconds and returned {} actions".format(t3 - t2, len(
                optimal_action_args_by_action)))
            self.logger.info("Recursion took {} seconds and explored {} actions".format(t4 - t3, explored_actions))
            self.logger.info("Action Breakdown:")
            for action in sorted(action_args_by_action, key=lambda x: -len(action_args_by_action[x])):
                self.logger.info("    {}: {}".format(len(action_args_by_action[action]), action.name))
